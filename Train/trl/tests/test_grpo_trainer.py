...
class GRPOTrainerTester(unittest.TestCase):
    ...
    def test_training_peft(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Check that the peft params have changed and the base_env model params have not changed
            for n, param in previous_trainable_params.items():
                new_param = trainer.model.get_parameter(n)
                if n in base_param_names:  # We expect the base_env model params to be the same
                    self.assertTrue(torch.allclose(param, new_param), f"Parameter {n} has changed.")
                elif "base_layer" not in n:  # We expect the peft params to be different (except for the base_env layer)
    ...
    def test_training_peft_with_gradient_checkpointing(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Check that only LoRA parameters have changed, base_env model parameters remain unchanged
            for n, param in previous_trainable_params.items():
                new_param = trainer.model.get_parameter(n)
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for n, param in previous_trainable_params.items():
                ...
                if n in base_param_names:  # We expect the base_env model params to be the same
                    ...
                    # We expect the peft params to be different (except for the base_env layer)
                    self.assertFalse(torch.allclose(param, new_param), f"Parameter {n} has not changed.")
