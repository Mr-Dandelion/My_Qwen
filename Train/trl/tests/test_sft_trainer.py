...
class SFTTrainerTester2(unittest.TestCase):
    ...
    def test_train_peft_model(self):
        # Get the base_env model
        model_id = "trl-internal-testing/tiny-Qwen2ForCausalLM-2.5"
        model = AutoModelForCausalLM.from_pretrained(model_id)

        # Get the base_env model parameter names
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Check the peft params have changed and the base_env model params have not changed
            for n, param in previous_trainable_params.items():
                new_param = trainer.model.get_parameter(n)
                if n in base_param_names:  # We expect the base_env model parameters to be the same
                    self.assertTrue(torch.allclose(param, new_param), f"Parameter {n} has changed")
                elif (
                    "base_layer" not in n
                ):  # We expect the peft parameters to be different (except for the base_env layer)
                    self.assertFalse(torch.allclose(param, new_param), f"Parameter {n} has not changed")
